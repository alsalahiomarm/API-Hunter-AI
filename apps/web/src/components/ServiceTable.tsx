      ...
      <table className={`w-full border-collapse text-sm ${isRtl ? "text-right" : "text-left"}`}>
        <thead>
          <tr className="border-b border-white/10 bg-night-800/60 text-xs font-black uppercase tracking-wide text-slate-400">
            <th className="px-4 py-3.5">{t("table.service")}</th>
            <th className="px-4 py-3.5">{t("table.category")}</th>
            <th className="w-[30%] px-4 py-3.5">{t("table.freePlan")}</th>
            <th className="px-4 py-3.5">{t("table.status")}</th>
            <th className="px-4 py-3.5">{t("table.codeExample")}</th>
            <th className="px-4 py-3.5">{t("table.actions")}</th>
          </tr>
        </thead>
        <tbody>
          {services.map((s) => (
            ...
            >
              <td className="px-4 py-4">
                ...
                <p className="mt-1.5 text-xs leading-5 text-slate-400">{localizedDescription(s, lang)}</p>
              </td>
              <td className="px-4 py-4">
